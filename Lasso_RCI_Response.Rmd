...
#         Depression_Responder ~ 
#       # predictors
#         Married + Days_not_attended_1 + Mood_D + Anxiety_C + Substance_C +
#         CUDOS_pre + CUXOS_pre + rdq_coping_change + rdq_pmh_change + 
#         rdq_func_change,
#       #data
#       data = Vars_dep, 
...
## min model
  glm_min_anx <-  
      ...
      # predictors
        ...
        Anxiety_D + Trauma_D + Substance_C + BPD_Co_C +
...
FP_test <- read_csv("~/Downloads/FP.min.anx")

FP_test$order = c(seq(from=2, to=length(FP.min.anx$Variable) + 1,1))
...
FP_test_m <- FP_test |>
  ...
  geom_point(aes(x=OR), shape= FP_test$sig,
               # ifelse(FP_test$p.adjusted <= 0.05, 
  ...
  coord_cartesian(ylim = c(1,length(FP_test$Variable) + 1)) +
  ...
  annotate("text", x = .8, y = length(FP_test$Variable) + 1, label = "Less Likely", size= 3) +
  annotate("text", x = 1.2, y = length(FP_test$Variable) + 1, label = "More Likely",size= 3) +
...
#arange the labels on top
      FP_test<-mutate(FP_test, top_row = 0)
      FP_test[14,12] = 1
      FP_test <- arrange(FP_test,order)
FP_test
    # plot the left side of the plot 
      FP_test_L <- FP_test %>% 
        ...
        hjust = 0,
        fontface = ifelse(FP_test$estimate_lab == "Odds Ratio (95% CI)", "bold","plain")) +
        theme_void() +
        coord_cartesian(ylim = c(1,length(FP_test$Variable) + 1), xlim = c(0, 4))
...
FP_test_R <-  
      ...
      ggplot() +
        ...
        fontface = ifelse(FP_test$p.adjusted_fig == "p value", "bold", "plain")) +
        theme_void() +
        coord_cartesian(ylim = c(1,length(FP_test$Variable) + 1))
...
FP_test_L + FP_test_m + FP_test_R + plot_layout(design = layout)
