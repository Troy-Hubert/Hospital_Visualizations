...
#Simplified lasso
lasso1_dep <- glmnet(x.dep, y.dep,family = "binomial", alpha = 1, lambda = NULL)
...
set.seed(123) 
cv.lasso.dep <- cv.glmnet(x.dep, y.dep, alpha = 1, family = "binomial")
...
model.min <- glmnet(x.dep, y.dep, alpha = 1, family = "binomial",
                lambda = cv.lasso.dep$lambda.min)
...
lasso.model.1se.dep <- glmnet(x.dep, y.dep, alpha = 1, family = "binomial",
                      lambda = cv.lasso.dep$lambda.1se)
...
## min model
  glm_min_dep <-
      ...
      # predictors
        ...
        CUXOS_pre + rdqPOST_coping + rdqPOST_pmh + rdqPOST_func + rdqPOST_wbs,
      #data
        data = Vars_dep,
...
####### JT - substance disorder seperate    
  # glm(
  #     #Outcome varaible
  #       Depression_Responder ~
  #     # predictors
  #       Age_1 + Some_College + Married + White + Days_complete_1 +  
  #       Mood_D + Substance_D + Mood_C + Substance_C + CUDOS_pre + 
  #       CUXOS_pre + rdqPOST_coping + rdqPOST_pmh + rdqPOST_func + rdqPOST_wbs,
  #     #data
  #       data = Vars_dep,
  #     # type of regression : logistic = binary / binomial
...
###### JT + substance in other
      # glm(
      # #Outcome varaible
      #   Depression_Responder ~
      # # predictors
      #   Days_complete_1 + Days_not_attended_1 + Other + Mood_C + CUDOS_pre +
      #   CUXOS_pre + rdqPOST_coping + rdqPOST_pmh + rdqPOST_func + rdqPOST_wbs,
      # #data
      #   data = Vars_dep,
      # # type of regression : logistic = binary / binomial
...
#Simplified lasso
lasso1_anx <- glmnet(x, y,family = "binomial", alpha = 1, lambda = NULL)
...
# Find the best lambda using cross-validation
set.seed(123) 
cv.lasso.anx <- cv.glmnet(x, y, alpha = 1, family = "binomial")
...
model <- glmnet(x, y, alpha = 1, family = "binomial",
                lambda = cv.lasso.anx$lambda.min)
...
# Some_College + Married + White + Days_complete_1 + Days_not_attended_1 + 
#         Anxiety_D + Other + Mood_C + BPD_Co_C + Substance_C + Trauma_C + Current_Dx +
#         CUDOS_pre + CUXOS_pre + rdq_coping_change + rdq_pmh_change + rdq_func_change,
...
## min model
  glm_min_anx <-  
      ...
      # predictors
        ...
        Anxiety_D + Trauma_D +  BPD_Co_C + CUXOS_pre +
...
## 1se model
  glm_1se_anx <-  
      ...
      #Outcome varaible
        Anxiety_Responder ~ 
      # predictors
        Days_complete_1 + Days_not_attended_1 + Anxiety_D +CUXOS_pre + rdqPOST_coping + rdqPOST_pmh + rdqPOST_func,
      #data
      data = Vars_anx, 
      # type of regression : logistic = binary / binomial
      family = "binomial")
...
# Coefficients
  coefs.1se.anx <- abs(coef(glm_1se_anx))

# Odds ratio
  OR.1se.anx <- exp(coef(glm_1se_anx))
...
FP_anx$Variable = c(
                    ...
                    "Anxiety Disorder", "Trauma Disorder", "Co-Occuring BPD",
                    "CUXOS Pre-Score",
