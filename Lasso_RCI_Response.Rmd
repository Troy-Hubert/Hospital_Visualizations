...
Mothership_lasso_anx <- Mothership_post_anx |>
  ...
  select( c(
    ...
    CUDOS_pre,CUXOS_pre, #CUDOS_first,CUXOS_first
    # RDQ
    rdqPOST_coping:rdqPOST_wbs,rdqPRE_coping:rdqPRE_wbs,
    #For table 3 - RCI info
    CUXOS_pre, CUXOS_post, Delta_Anx_DC, CUDOS_pre, CUDOS_post, Delta_Dep_DC,
        #ACT stuff
    AAQIIpre, AAQIIpost, 
    Desc_Ex_post, Non_react_post, Observe_inner_post, Self_compasion_post,
    Acting_mindful_post, FFMQ_post_total, 
    Desc_Ex_pre, Non_react_pre, Observe_inner_pre, Self_compasion_pre, 
    Acting_mindful_pre, FFMQ_post_total
  ))
...
glm_min_dep <-
    ...
    #Outcome varaible
      Depression_Responder ~
    # predictors
Days_not_attended_1 + Anxiety_C + BPD_Co_C + Current_Dx + 
  CUXOS_pre +  Non_react_post + 
  Self_compasion_post + Acting_mindful_post,
    #data
      data = Vars_dep,
    # type of regression : logistic = binary / binomial
      family = "binomial",
...
# R^2 values  
  ...
  #wald
   library(aod)
   # wald.test(b = coef(glm_min_dep),Sigma = vcov(glm_min_dep),Terms= 2:13)
...
## min model
  glm_min_anx <-  
      ...
      #Outcome varaible
        Anxiety_Responder ~ 
      # predictors
        Age_1 + Some_College + Female + Married + White + Days_not_attended_1 + 
        BPD_Co_C + CUXOS_pre +  
        Non_react_post+  Self_compasion_post + Acting_mindful_post,
      #data
      data = Vars_anx, 
      # type of regression : logistic = binary / binomial
      family = "binomial")
...
#wald
   library(aod)
   wald.test(b = coef(glm_min_anx),Sigma = vcov(glm_min_anx),Terms= 2:12)
...
layout <- c(
  ...
  patchwork::area(t = 0, l = 19, b = 30, r = 30),
