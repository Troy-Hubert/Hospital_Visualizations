...
summary(Mothership_lasso$Depression_Responder)
...
```{r}
#### Redo and recreate dataset for Anxiety or depression response.
...
print(paste0("This is the minimum lambda value: ", round(cv.lasso.dep$lambda.min,5)))

#This value is called lambda.1se.
print(paste0("This is the 1 standard error lambda value: ", round(cv.lasso.dep$lambda.1se,5)))
...
## min model
  glm_min_dep <-
      ...
      #Outcome varaible
        Depression_Responder ~
      # predictors
        White + Days_complete_1 + Days_not_attended_1 + Mood_D + Substance_D + 
        Other + Anxiety_C + Mood_C + Substance_C + CUDOS_pre + CUXOS_pre + 
...
Preparing data for lasso - Anxiety
```{r}
Mothership_lasso_anx <- Mothership_post |>
    filter(
    !is.na(Some_College),
    !is.na(Female),
    !is.na(IPT_track_1),
...
## min model
  glm_min_anx <-  
      ...
      #Outcome varaible
        Anxiety_Responder ~ 
      # predictors
        Married + White + Days_complete_1 + Days_not_attended_1 + 
        Anxiety_D + Mood_C + Substance_C + Trauma_C +  CUDOS_pre + CUXOS_pre + 
        rdq_coping_change + rdq_pmh_change + rdq_func_change,
      #data
      data = Vars_anx, 
      # type of regression : logistic = binary / binomial
      family = "binomial")
...
FP.min.anx$order = c(rev(seq(length(FP.min.anx$Variable),1)))
FP.min.anx
...
FP.min.anx |>
  ...
  #coord_cartesian(ylim = c(1,length(FP.min.anx$Variable))) +
  #add a line at 0 for reference 
    
  #add anotations to help suggests what each side means.
  # annotate("text", x = .5, y = 14, label = "Less Likely", size= 3) +
  # annotate("text", x = 1.5, y = 14, label = "More Likely",size= 3) +
