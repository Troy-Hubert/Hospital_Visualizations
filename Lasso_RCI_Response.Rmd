...
# Make prediction on test data
x.test <- model.matrix(Depression_Responder ~., test.data)[,-1]
probabilities <- lasso.model.1se.dep %>% predict(newx = x.test,type ="response")
predicted.classes <- ifelse(probabilities > 0.5, "Responder", "Non-Responder")
...
write_csv(dep_df, "~/Downloads/dep_df")
```
...
ANX_FP <- 
  ...
  geom_linerange(aes(xmin=OR_low, xmax=OR_high),size = 1, show.legend = FALSE,
                color = ifelse(anx_df$p.adjusted < 0.05, "red", "darkgrey")) +
  geom_point(aes(x=OR), shape= 16,
             size=4,show.legend = FALSE,
             color = ifelse(anx_df$p.adjusted < 0.05, "red", "darkgrey")) +
  geom_point(aes(x=OR), shape= 1,
             size=4,show.legend = FALSE, color = "white") +
    geom_point(aes(x=OR), shape= 1,
             size=4.1,show.legend = FALSE, color = "white")+
...
DEP_FP <- 
  ...
  #make the forrest plot
  geom_linerange(aes(xmin=OR_low, xmax=OR_high),size = 1, show.legend = FALSE,
                color = ifelse(dep_df$p.adjusted < 0.05, "blue", "darkgrey")) +
  geom_point(aes(x=OR), shape= 16,
             size=4,show.legend = FALSE,
             color = ifelse(dep_df$p.adjusted < 0.05, "blue", "darkgrey")) +
  geom_point(aes(x=OR), shape= 1,
