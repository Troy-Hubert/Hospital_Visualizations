...
summary(as.factor(Mothership$Prev_AdmitID_1))
view(Mothership)
```
...
glm_min_dep <-
    ...
    #Outcome varaible
      Depression_Responder ~
    # predictors
      Some_College + Female + White + YA_Treatment + Trauma_Treatment + Days_complete_1 + 
      Days_not_attended_1 + Anxiety_D + BPD_P+ Trauma_D+Other+ Anxiety_C + Mood_C+ 
      BPD_Co_C + Substance_C + No_Co + Current_Dx + CUXOS_pre + rdqPRE_coping + 
      rdqPRE_pmh + rdqPRE_func + AAQIIpre + Self_compasion_pre,
    #data
      data = Vars_dep,
    # type of regression : logistic = binary / binomial
      family = "binomial",
...
#       Depression_Responder ~
#     # predictors
#       White  +  Days_not_attended_1 + Anxiety_C + Current_Dx + CUXOS_pre +
#       Non_react_post + AAQIIpost + Acting_mindful_post,
#     #data
#       data = Vars_dep,
#     # type of regression : logistic = binary / binomial
#       family = "binomial",
...
# R^2 values  
  ...
  #wald
   library(aod)
   wald.test(b = coef(glm_min_dep),Sigma = vcov(glm_min_dep),Terms= 2:24)
...
Vars_anx <- Vars_anx |>
  ...
  glm_min_anx <-  
      ...
      #Outcome varaible
        Anxiety_Responder ~ 
      # predictors
        Age_1 + Female + Married + White + YA_Treatment + Trauma_Treatment + 
        Days_complete_1 + Days_not_attended_1 + Anxiety_D + Mood_C + Trauma_C + 
        Substance_C + Current_Dx + CUDOS_pre + rdqPRE_coping + rdqPRE_func + 
        AAQIIpre + Non_react_pre + Self_compasion_pre,
      #data
      data = Vars_anx, 
      # type of regression : logistic = binary / binomial
      family = "binomial")
...
#wald
   library(aod)
   wald.test(b = coef(glm_min_anx),Sigma = vcov(glm_min_anx),Terms= 2:20)
