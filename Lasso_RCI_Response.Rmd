...
```{r}
Mothership_lasso_1 <- Mothership_post |>
    filter(
    !is.na(Some_College),
    !is.na(Female),
    !is.na(IPT_track_1),
  ...
  select( c(
   # Anxiety_Responder, Depression_Responder,
    # Demographics
    ID1, Age_1,Some_College:Married,White, #Discharge,
    # Treatment info
    IPT_track_1, Days_complete_1, Days_not_attended_1, #Days_cancel_1,
    # Disorders
    Anxiety_D, Mood_D, BPD_P, Trauma_D, 
...
Mothership_lasso_1 <- na.omit(Mothership_lasso_1)
```
...
summary(Mothership_lasso$Anxiety_Responder)
...
#     # Disorders
#     Anxiety_D, Mood_D, BPD_P,    Trauma_D, Substance_D, Psychotic_D,  Other,
#     Anxiety_C, Mood_C, BPD_Co_C, Trauma_C, Substance_C, Other_C, No_Co,
#     Current_Dx,
#     # Pre-Post stuff
#     CUDOS_pre,CUXOS_pre,
...
table1_vars_anx <- c(colnames(Mothership_lasso)[-1])
...
table1_tab_anx <- CreateTableOne(vars = table1_vars_anx, data = Vars_anx,strata ="Anxiety_Responder",  factorVars = catVars)
...
## Vector of variables to summarize
table1_vars_dep <- c(colnames(Vars_dep)[-1])

## Vector of categorical variables that need transformation
catVars <- c("Some_College", "Female" , "Married", "White",
             "IPT_track_1","Anxiety_D", "Mood_D", "BPD_P" ,"Trauma_D",        
             "Psychotic_D", "Other", "Anxiety_C", "Mood_C" , "BPD_Co_C", "Trauma_C", 
             "Substance_C", "Other_C" , "No_Co"
             )
## Create a TableOne object
table1_tab_dep <- CreateTableOne(vars = table1_vars_dep, data = Vars_dep,strata ="Depression_Responder",  factorVars = catVars)
...
table1_gen <- CreateTableOne(vars = table1_vars_dep, data = Mothership_lasso,  factorVars = catVars)
...
pre_post_plot<- ggplot(data=AnxDep_fig_response,
...
Dis_tab <- data.frame(table1::table1(~Disorder_Type + Current_Dx | Anxiety_Responder, 
                                     data = Mothership_lasso))
colnames(Dis_tab) = c("Variables","Anxiety Non Responder", "Anxiety Responder", "Total")

Dis_tab1 <- data.frame(table1::table1(~Disorder_Type + Current_Dx | Depression_Responder, 
                                     data = Mothership_lasso))
...
Vars_FULL <- na.omit(Vars_dep)

# install.packages("ggcorrplot")
library(ggcorrplot)
model.matrix(~0+., data=Vars_dep) %>% 
...
## 1se model
  glm_1se_anx <-  
      ...
      #Outcome varaible
        Anxiety_Responder ~ 
      # predictors
        Days_not_attended_1 + CUXOS_pre + rdqPOST_coping + rdqPOST_pmh + rdqPOST_wbs,
      #data
      data = Vars_anx, 
      # type of regression : logistic = binary / binomial
      family = "binomial")
...
FP_anx$Variable = c(
                    "Age", "Married", "Days Completed", "Days Not Attended", 
