...
#2. Create pre_scores
      ...
      CUDOX_pre = Day1_CUDOS
      )

#3. Then create df with CUDOS and CUXOS scores
  # Depression
  ...
  # Anxiety
    ...
    CUXOS_Score <- data.frame(CUDOS_Score)
    
    view(Mothership)
    
#4. Extract the valeues for each patient at their last day (length of stay), and create a CUDOS and CUXOS post score.

Mothership <- Mothership %>%
  mutate(
    CUDOS_post = CUDOS_Score[cbind(seq_along(1:8667),CUDOS_Score$Days_complete_W)],
    CUXOS_post = CUXOS_Score[cbind(seq_along(1:8667),CUDOS_Score$Days_complete_W)]
  )

#5. create new DF with Varaibles we want

AnxDep_PrePost<- data.frame(cbind(Mothership$ID1,Mothership$Disorder_Type,CUDOS_pre,CUDOS_post,CUXOS_pre,CUXOS_post)) %>%
  filter(!is.na(Mothership$Disorder_Type))
colnames(AnxDep_PrePost)[1:2] = c("ID1","Disorder_Type")
...
```
Anxiety / Depression Pre_Post by disorder - line plot
```{r}
...
Depression/ Anxiety pre/ post Dumbell plots
```{r}
