...
head(df)
mode(Mothership1$Relationship_1)
```
...
Mothership<-Mothership %>%
  mutate(
          ...
          #this isn't working???
                  # Sum_DX = #Primary
                  #       as.integer(Mood_D) + as.integer(Anxiety_D) +as.integer(OCD_P) + 
                  #       as.integer(BPD_P) + as.integer(Substance_D) + as.integer(Trauma_D) + 
                  #       as.integer(Psychotic_D) + as.integer(Eating_D) + as.integer(Somatic_D) + as.integer(Other) +
                  #          #Co-Occurring
                  #       as.integer(GAD_C) + as.integer(Panic_C) + as.integer(PanicAgor_C) + as.integer(Agoraphobia_C) + 
                  #       as.integer(Social_C) + as.integer(Specific_C) + as.integer(MDD_C) + as.integer(Bipolar1_C) + as.integer(Bipolar2_C) + 
                  #       as.integer(Dysthymia_C) + as.integer(Alcohol_C) + as.integer(Drug_C) + as.integer(PTSD_C) + 
                  #       as.integer(Adjustment_C) + as.integer(Schiz_C) + as.integer(Schizaff_C) + as.integer(Anorex_C) + as.integer(Bulim_C) + 
                  #       as.integer(Binge_C) + as.integer(Somat_C) + as.integer(UndiffSoma_C) + as.integer(Hypochondriasis_C) + as.integer(OCD_Co_C) + 
                  #       as.integer(BPD_Co_C)
                      )
...
Part 1 - treatment information

- demographics breakdown
- treatment breakdown
    - duration of stay based upon disorder?
    - mean anx / depression based upon stay
- Treatment discharge?
- pre post by responder non responder
...
resp_tab <- data.frame(table1::table1(
  ~IPT_track_1 + Days_complete_1 + DC_status_1 + Days_cancel_1 + Days_not_attended_1 + Days_noshow_1 + Current_Dx | RESPONDER_FIGS, 
...
summary(Mothership_post$Duration_lab)
test <- Mothership_post |>
  filter(
Duration == 2
)
...
FIGURE OUT WHY THIS SHIT ISN'T WORKING IT IS GRABBING VALUES THAT DOEN'T EXIST
ITS GRABBING VALUES (CUXOS) FOR PEOPLE THAT DISCHARGED. I DON'T UNDERSTAND HOW THIS WORKS
```{r}
Mothership_duration_CUXOS<-Mothership_Mean_CUXOS[1:83,]
View(Mothership_duration_CUXOS)
...
ggplot(data=Mothership_duration_CUXOS,aes(x=TxDay,y=mean_CUXOS,group=Duration_lab,color=Duration_lab))+
  geom_line(size=1)+
  geom_point()+
  theme(panel.grid.major.y = element_line(color = "grey",size = 0.5,linetype = 2))+
  ggtitle("Mean Anxiety Severity During Partial Hospitalization")+
...
### Education Status
```{r}
ggplot(data = subset(Mothership_post, !is.na(Education_1)),mapping = aes(x = Education_1))+
...
### Race
```{r}
ggplot(data = subset(Mothership_post, !is.na(Race_1)),mapping = aes(x = Race_1))+
...
### Gender
```{r}
ggplot(data = subset(Mothership_post, !is.na(Gender_1)),mapping = aes(x = Gender_1))+
...
Mothership_post$Relationship_1

ggplot(data = subset(Mothership_post, !is.na(Relationship_1)),mapping = aes(x = Relationship_1))+
  geom_bar(color = "black",fill="darkorange")+
  ggtitle("Frequency of Relationship") +
  theme(axis.text.x = element_text(angle = 45,hjust = 1))+
  xlab("relationshipr")+
  theme(panel.grid.major.y = element_line(color = "grey",size = 0.5,linetype = 2))+
  theme(panel.background=NULL)+
...
summary(Mothership_post$Relationship_1)
...
ggplot(data = subset(Mothership_post, !is.na(IPT_track_1)),mapping = aes(x = IPT_track_1))+
...
ggplot(data = subset(Mothership_post, !is.na(TxYear)),mapping = aes(x = TxYear))+
...
```{r}
### Duration of Treatment ###
ggplot(data = subset(Mothership_post, !is.na(Duration)),mapping = aes(x = Duration))+
...
tables
```{r}
  ### To univariate tests with these???

#Disorder Table - duratin
# Dis_tab <- data.frame(table1::table1(~Disorder_Type | Duration, data = Mothership_post))
#                                  #Mothership_Diag_treat))
# colnames(Dis_tab) <- c("Primary Diagnosis","1-5 Days", "6-10 Days", "11-15 Days","16+ Days", "Total Sample")
# Dis_tab

#Disorder Tab by 10 days
# Dis_tab_10 <- data.frame(table1::table1(~Disorder_Type | Duration_10, data = Mothership_post))
# colnames(Dis_tab_10) <- c("Primary Diagnosis","1-10 Days", "10+ Days", "Total Sample")
# Dis_tab_10

# Disorder by responder
```



Dumbell Plot by Disorder - Pre and Post
```{r}
AnxDep_PrePost <- data.frame(cbind(Mothership_post$ID1,
...
colnames(AnxDep_PrePost) = c("ID1","Disorder_Type", "CUDOS_pre","CUDOS_post","CUXOS_pre","CUXOS_post","RESPONDER_FIGS)")
...
Education<-table(Mothership_post$Education_1)
