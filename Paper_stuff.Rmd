...
# 3. Getting BL scores for people who have NA at Baseline 
    ...
    dim(CUDOS_Score_pre)
    dim(Mothership_post)
    length(Mothership_post$BL_CUDOS)
    length(Pre_dep)
    ...
    Mothership_post <- Mothership_post |>
      mutate(
        ...
        Sig_unresponse = as.factor(ifelse(Sig_BL %in% c("Sig Both","Sig Anx","Sig Dep") & Sig_Post == "Subclinical", "Responder",
                            ...
                            ifelse(Sig_BL == "Sig Both" & Sig_Post == "Sig Dep", "Anx gone Dep not", NA
                            ))))))))
                            
  # checking for pre scores
   (grepl("Sig Both",Mothership_post$Sig_pre) | grepl("Sig Anx",Mothership_post$Sig_pre) | grepl("Sig Anx",Mothership_post$Sig_pre))
    &
  # Checking psot scores
  (grepl("Sig Both",Mothership_post$Sig_Post) | grepl("Sig Anx",Mothership_post$Sig_Post) | grepl("Sig Anx",Mothership_post$Sig_Post))
