...
AnxDep_PrePost <- data.frame(cbind(Mothership_post$ID1,
                                  ...
                                  #Extra stuff?
                                  Mothership_post$BL_Missing,
                                  Mothership_post$Sig_BL,
                                  Mothership_post$Sig_Post,
                                  Mothership_post$Sig_unresponse,
                                  Mothership_post$Sig_unresponse_type
                                  )) #%>%
  #filter(!is.na(Mothership_post$Disorder_treat))

colnames(AnxDep_PrePost) = c("ID1","Disorder_Type", "BL_CUDOS", "BL_CUXOS", "CUDOS_pre","CUDOS_post","CUXOS_pre","CUXOS_post","BL_Missing",
                             "Sig_BL", "Sig_Post","Sig_unresponse", "Sig_unresponse_type")
...
view(data.frame(table1::table1(~BL_CUXOS + BL_CUDOS + SUManx_DC + SUMdep_DC + CUDOS_post +CUXOS_post, data = Mothership_post)))
...
AnxDep_response_mean<-AnxDep_PrePost_Long %>%
  filter(!is.na(Sig_unresponse)) |>
  group_by(Test,Sig_unresponse,PrePost) %>%
  summarise(Score=mean(Score,na.rm=TRUE))
...
ggplot(data=AnxDep_response_mean,
  ...
  theme(panel.background=NULL)+
  theme(legend.title = element_blank()) +
  facet_wrap(~Sig_unresponse,labeller = labeller(Sig_unresponse =
      c("1" = "Non-responders", 
        "2" = "Responders")
      ))
```
...
AnxDep_Treat_mean<-AnxDep_PrePost_Long %>%
  filter(!is.na(Sig_unresponse),
         !is.na(Disorder_Type)) |>
  group_by(Test,Disorder_Type,Sig_unresponse,PrePost) %>%
  summarise(Score=mean(Score,na.rm=TRUE))
...
p <- ggplot(data=AnxDep_Treat_mean,
  ...
  theme(panel.background=NULL)+
  theme(legend.title = element_blank()) +
  facet_grid(Sig_unresponse~Disorder_Type,
             labeller = labeller(
               Sig_unresponse = c("1" = "Non-responders", "2" = "Responders"),
...
Mothership_post <- Mothership_post |>
  ...
  mutate(
    ...
    Anx_Responder = as.factor(
      ifelse(CUXOS_pre > 10 & Percent_Anx_change  <= -50 | CUXOS_post < 20, "Anx Responder",
      ifelse(CUXOS_pre > 10 & Percent_Anx_change > 0, "Anx Worse", 
      ifelse(CUXOS_pre > 10 & Percent_Anx_change < 0 & Percent_Anx_change > -50 & CUXOS_post < 20,  "Unresponders", "Low BL Anx"      
      )))),
...
dataRci <- rci(pre="pre", post="post", data=RCI_df_anx, rtt=.74, sdNorm=19.5)
ggplot(dataRci, aes(x = rci)) +
  geom_histogram(fill = "blue", alpha = .5) +
  labs(
    title = "Histogram of RCI",
    xlab = "RCI",
    ylab = "Quanity of patients"
  ) 
...
The mean total score on the CUXOS was 25.8 (SD = 19.5). The mean scores on the 6-item psychic anxiety subscale and 14-item somatic anxiety subscale were 10.6 (SD = 7.1) and 15.3 (SD = 13.7), respectively.

A factor analysis yielded 2 factors with eigenvalues greater than 1. The 2 factors accounted for 58.7% of the total variance. On the first factor, the 14 somatic anxiety items had the highest loadings, each item with a loading ≥ 0.49. On the second factor, the 6 psychic anxiety items had the highest loadings with each item loading ≥ 0.68.

The internal consistency of the CUXOS total scale score and the psychic and somatic anxiety subscales was examined in the entire sample who completed the scale at the initial evaluation and a sample of 120 patients who completed it at a follow-up appointment. The CUXOS demonstrated excellent internal consistency (baseline: Cronbach α = .95 for the total scale, .90 for the psychic anxiety subscale, and .93 for the somatic anxiety subscale; follow-up: α = .95 for the total scale, .94 for the psychic anxiety subscale, and .91 for the somatic anxiety subscale). The data in Table 2 show the correlation between each item and the total scale score. All item-scale correlations were statistically significant at baseline (median = 0.68) and at follow-up (median = 0.68).

The test-retest reliability of the CUXOS was examined in 216 subjects who completed the scale at the time of their initial appointment. The test-retest reliability of the total scale and the psychic and somatic anxiety subscales was high (r = 0.90, 0.86, 0.88, respectively), and the test-retest reliability of each item was statistically significant (median r = 0.75) (Table 2).
